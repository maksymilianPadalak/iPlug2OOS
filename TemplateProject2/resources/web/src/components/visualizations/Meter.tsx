...
export function Meter({ channel, compact = false }: MeterProps) {
  ...
  return (
    <div className="flex items-center gap-2 bg-gradient-to-br from-stone-900/60 to-black/60 border border-cyan-700/30 rounded-lg px-4 py-2 shadow-md">
      {/* Numeric display */}
      <div className={`text-lg font-black font-mono transition-colors ${
        isClipping ? 'text-red-400' : isHot ? 'text-cyan-400' : 'text-cyan-300/70'
      }`}>
