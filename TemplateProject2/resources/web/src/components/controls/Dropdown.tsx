  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <select
        ...
        className="bg-gradient-to-b from-stone-800 to-stone-900 border border-cyan-600/50 text-cyan-100 px-3 py-2 rounded font-bold text-xs uppercase tracking-wider focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 hover:from-stone-700 hover:to-stone-800 cursor-pointer transition-all shadow-md"
      >
        {options.map((option, index) => (
          <option key={index} value={index} className="bg-stone-900 text-cyan-100">
            {option}
          </option>
        ))}
      </select>
