...
NOTARIZE_BUNDLE_ID=com.AcmeInc.TemplateProject2
NOTARIZE_BUNDLE_ID_DEMO=com.AcmeInc.TemplateProject2.DEMO
...
# Use TemplateProject-macOS.xcodeproj (the actual project name in TemplateProject2)
# Build AU target only
# Build without xcpretty if not available
if command -v xcpretty &> /dev/null; then
  xcodebuild -project ./projects/TemplateProject-macOS.xcodeproj -xcconfig ./config/TemplateProject-mac.xcconfig DEMO_VERSION=$DEMO -target "AU" -UseModernBuildSystem=NO -configuration Release | tee build-mac.log | xcpretty
else
  echo "⚠️  xcpretty not found, building without it..."
  xcodebuild -project ./projects/TemplateProject-macOS.xcodeproj -xcconfig ./config/TemplateProject-mac.xcconfig DEMO_VERSION=$DEMO -target "AU" -UseModernBuildSystem=NO -configuration Release 2>&1 | tee build-mac.log
fi
...
if [ -d $AU ]; then
  ./$SCRIPTS/SetFileIcon -image resources/$PLUGIN_NAME.icns -file $AU 2>/dev/null || true
fi

if [ -d $VST2 ]; then
  ./$SCRIPTS/SetFileIcon -image resources/$PLUGIN_NAME.icns -file $VST2 2>/dev/null || true
fi

if [ -d $VST3 ]; then
  ./$SCRIPTS/SetFileIcon -image resources/$PLUGIN_NAME.icns -file $VST3 2>/dev/null || true
fi

if [ -d $CLAP ]; then
  ./$SCRIPTS/SetFileIcon -image resources/$PLUGIN_NAME.icns -file $CLAP 2>/dev/null || true
fi

if [ -d "${AAX}" ]; then
  ./$SCRIPTS/SetFileIcon -image resources/$PLUGIN_NAME.icns -file "${AAX}" 2>/dev/null || true
fi
...
if [ $BUILD_INSTALLER == 1 ]; then
  ...
  ./scripts/makeinstaller-mac.sh $FULL_VERSION 2>/dev/null || echo "⚠️  Installer script not found, skipping"
  ...
  #set installer icon
  ./$SCRIPTS/SetFileIcon -image resources/$PLUGIN_NAME.icns -file "${PKG}" 2>/dev/null || true
  ...
  if [ -d installer/$PLUGIN_NAME.dmgCanvas ]; then
    ...
    cp installer/changelog.txt build-mac/installer/ 2>/dev/null || true
    cp installer/known-issues.txt build-mac/installer/ 2>/dev/null || true
    cp "manual/$PLUGIN_NAME manual.pdf" build-mac/installer/ 2>/dev/null || true
...
echo ""
zip -r ./build-mac/$ARCHIVE_NAME-dSYMs.zip ./build-mac/Release/*.dSYM 2>/dev/null || true
