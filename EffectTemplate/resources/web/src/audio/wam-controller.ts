...
export async function initializeWAM(): Promise<(WAMController & { audioContext: AudioContext }) | null> {
  ...
  console.log('EffectTemplateController exists:', !!(window as any).EffectTemplateController);
  ...
  try {
    ...
    if (!(window as any).EffectTemplateController) {
      console.log('📥 Loading EffectTemplate-awn.js...');
      await loadScript('scripts/EffectTemplate-awn.js');
    } else {
      console.log('✅ EffectTemplate-awn.js already loaded');
    }
  } catch (error) {
  ...
  if (!window.EffectTemplateController) {
    console.error('EffectTemplateController not found after loading scripts');
  ...
  await window.EffectTemplateController.importScripts(actx);
  console.log('📦 EffectTemplateController.importScripts completed');

  const wamController = new window.EffectTemplateController(actx, options);
...
function loadScript(src: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (src.includes('EffectTemplate-awn.js') && (window as any).EffectTemplateController) {
      console.log(`✅ ${src} already loaded (EffectTemplateController found)`);
      resolve();
      return;
    }
    ...
    if (existingScript) {
      ...
      const checkInterval = setInterval(() => {
        ...
        if (src.includes('wam-controller.js') && (window as any).WAMController) {
          ...
          resolve();
        } else if (src.includes('EffectTemplate-awn.js') && (window as any).EffectTemplateController) {
          clearInterval(checkInterval);
          console.log(`✅ ${src} loaded successfully`);
          resolve();
    ...
    script.onload = () => {
      ...
      if (src.includes('EffectTemplate-awn.js') && !(window as any).EffectTemplateController) {
        reject(new Error(`Script ${src} loaded but EffectTemplateController not found`));
        return;
      }
      console.log(`✅ ${src} loaded successfully`);
      resolve();
